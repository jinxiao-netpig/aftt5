...
class T5Attention(nn.Module):
    def __init__(self, config: T5Config, has_relative_attention_bias=False):
        ...
        self.key_value_proj_dim = config.d_kv
        self.n_heads = config.num_heads
        self.hidden_dim = self.n_heads * self.key_value_proj_dim
        self.dim = config.d_model
        self.q = nn.Linear(self.dim, self.hidden_dim)
        self.k = nn.Linear(self.dim, self.hidden_dim)
        self.v = nn.Linear(self.dim, self.hidden_dim)
        self.o = nn.Linear(self.hidden_dim, self.dim)
